...
*/
  ...
  class OSCOM_PayPal_PS_Cfg_prepare_order_status_id {
      ...
      if ( !defined('OSCOM_APP_PAYPAL_PS_PREPARE_ORDER_STATUS_ID') ) {
        ...
        if (mysqli_num_rows($check_query) < 1) {
          ...
          foreach (array_values(language::load_all()) as $lang) {
            $GLOBALS['db']->query("insert into orders_status (orders_status_id, language_id, orders_status_name) values ('" . $status_id . "', '" . $lang['id'] . "', 'Preparing [PayPal Standard]')");
          }
    ...
    function getSetField() {
      ...
      $input = (new Select('prepare_order_status_id', $statuses_array, ['id' => 'inputPsPrepareOrderStatusId']))->set_selection(OSCOM_APP_PAYPAL_PS_PREPARE_ORDER_STATUS_ID);
