...
*/
  ...
  class cm_i_modular extends abstract_executable_module {

    const CONFIG_KEY_BASE = 'MODULE_CONTENT_I_MODULAR_';

    public function __construct() {
      parent::__construct(__FILE__);
      
      $this->description .= cm_i_modular::display_layout();
    }
    ...
    function execute() {
      ...
      if ( defined('MODULE_CONTENT_I_INSTALLED') && !Text::is_empty(MODULE_CONTENT_I_INSTALLED) ) {
        $i_modules = array_filter(array_map(function ($im) {
          $class = pathinfo($im, PATHINFO_FILENAME);

          return new $class();
        }, explode(';', MODULE_CONTENT_I_INSTALLED)), function ($_i) {
          return $_i->isEnabled();
        });

        if ( count($i_modules) > 0 ) {
          array_walk($i_modules, function ($v, $unused) { $v->getOutput(); });
    ...
    public static function display_layout() {
      if ( !defined('MODULE_CONTENT_I_MODULAR_STATUS') ) {
        return null;
      }
      
      $slots = ['A' => ['width' => MODULE_CONTENT_I_MODULAR_A_WIDTH, 'color' => '96858f'],
                'B' => ['width' => MODULE_CONTENT_I_MODULAR_B_WIDTH, 'color' => '6d7993'],
                'C' => ['width' => MODULE_CONTENT_I_MODULAR_C_WIDTH, 'color' => '9099a2'],
                'D' => ['width' => MODULE_CONTENT_I_MODULAR_D_WIDTH, 'color' => 'd5d5d5'],
                'E' => ['width' => MODULE_CONTENT_I_MODULAR_E_WIDTH, 'color' => '96858f'],
                'F' => ['width' => MODULE_CONTENT_I_MODULAR_F_WIDTH, 'color' => '6d7993'],
                'G' => ['width' => MODULE_CONTENT_I_MODULAR_G_WIDTH, 'color' => '9099a2'],
                'H' => ['width' => MODULE_CONTENT_I_MODULAR_H_WIDTH, 'color' => 'd5d5d5'],
                'I' => ['width' => MODULE_CONTENT_I_MODULAR_I_WIDTH, 'color' => '96858f']];
    ...
    public function install($parameter_key = null) {
      if (!defined('MODULE_CONTENT_I_INSTALLED')) {
...
INSERT INTO configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added)
 VALUES ('Installed Modules', 'MODULE_CONTENT_I_INSTALLED', '', 'This is automatically updated. No need to edit.', 6, 0, NOW())
EOSQL
          );
      }
