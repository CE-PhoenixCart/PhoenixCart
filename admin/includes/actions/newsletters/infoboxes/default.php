    ...
    if ($nInfo->locked > 0) {
      $contents[] = [
        ...
        'text' => $GLOBALS['Admin']->button(IMAGE_EDIT, 'fas fa-cogs', 'btn-warning mr-2', (clone $link)->set_parameter('action', 'new'))
                . $GLOBALS['Admin']->button(IMAGE_DELETE, 'fas fa-trash', 'btn-danger mr-2', (clone $link)->set_parameter('action', 'delete')),
      ];
      $contents[] = [
        'class' => 'text-center',
        'text' => $GLOBALS['Admin']->button(IMAGE_PREVIEW, 'fas fa-eye', 'btn-light mr-2', (clone $link)->set_parameter('action', 'preview'))
      ...
      $contents[] = ['class' => 'text-center', 'text' => $GLOBALS['Admin']->button(IMAGE_SEND, 'fas fa-paper-plane', 'btn-success btn-block', $link->set_parameter('action', 'send'))];
    } else {
      $contents[] = [
        'class' => 'text-center',
        'text' => $GLOBALS['Admin']->button(IMAGE_PREVIEW, 'fas fa-eye', 'bt-info mr-2', (clone $link)->set_parameter('action', 'preview'))
