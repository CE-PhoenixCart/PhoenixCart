...
<?= new Form('review', (clone $form_link)->set_parameter('action', 'process'), 'post', ['class' => 'was-validated', 'enctype' => 'multipart/form-data']) ?>
  ...
  <div class="row mb-2">
    <label for="inputNickName" class="col-form-label col-sm-3 text-start text-sm-end"><?= SUB_TITLE_FROM ?></label>
    <div class="col-sm-9">
      <div class="input-group">
        <?= (new Input('nickname', ['value' => htmlspecialchars($customer->get('short_name')), 'id' => 'inputNickName', 'placeholder' => SUB_TITLE_REVIEW_NICKNAME]))->require(), FORM_REQUIRED_INPUT ?>
      </div>
    </div>
  </div>

  <div class="row mb-2">
    <label for="inputReview" class="col-form-label col-sm-3 text-start text-sm-end"><?= SUB_TITLE_REVIEW ?></label>
    <div class="col-sm-9">
      <div class="input-group">
        <?= (new Textarea('review', ['cols' => '60', 'rows' => '15', 'id' => 'inputReview', 'placeholder', SUB_TITLE_REVIEW_TEXT]))->require(), FORM_REQUIRED_INPUT ?>
      </div>
    </div>
  </div>
