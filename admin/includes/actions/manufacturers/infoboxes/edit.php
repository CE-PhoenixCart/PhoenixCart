...
EOSQL
    ...
    $manufacturer_inputs_string .= '<div class="input-group mb-1"><div class="input-group-prepend"><span class="input-group-text">' . $flag_image . '</span></div>' . new Input("manufacturers_url[{$l['id']}]", ['id' => "mUrl-{$l['code']}", 'value' => $manufacturer['manufacturers_url']]) . '</div>';
    $manufacturer_seo_title_string .= '<div class="input-group mb-1"><div class="input-group-prepend"><span class="input-group-text">' . $flag_image . '</span></div>' . new Input("manufacturers_seo_title[{$l['id']}]", ['id' => "mSeoTitle-{$l['code']}", 'value' => $manufacturer['manufacturers_seo_title']]) . '</div>';
    $manufacturer_description_string .= '<div class="input-group mb-1"><div class="input-group-prepend"><span class="input-group-text">' . $flag_image->set('style', 'vertical-align: top;') . '</span></div>' . (new Textarea("manufacturers_description[{$l['id']}]", ['cols' => '80', 'rows' => '10', 'id' => "mDescription-{$l['code']}"]))->set_text($manufacturer['manufacturers_description'] ?? '') . '</div>';
    $manufacturer_seo_description_string .= '<div class="input-group mb-1"><div class="input-group-prepend"><span class="input-group-text">' . $flag_image . '</span></div>' . (new Textarea("manufacturers_seo_description[{$l['id']}]", ['cols' => '80', 'rows' => '10', 'id' => "mSeoDescription-{$l['code']}"]))->set_text($manufacturer['manufacturers_seo_description'] ?? '') . '</div>';
  }
  ...
  if ($mInfo->in_eu ?? 0 == 1) $checkbox->tick();
  ...
  $contents[] = ['class' => 'hidden', 'text' => TEXT_IMPORTERS_NAME . '<br>' . new Input('importers_name', ['id' => 'iName', 'value' => $mInfo->importers_name])];
  $contents[] = ['class' => 'hidden', 'text' => sprintf(TEXT_IMPORTERS_ADDRESS, (new Textarea("importers_address", ['cols' => '80', 'rows' => '10', 'id' => "iAddress"]))->set_text($mInfo->importers_address ?? ''))];
  $contents[] = ['class' => 'hidden', 'text' => sprintf(TEXT_IMPORTERS_EMAIL, new Input('importers_email', ['id' => 'iEmail', 'value' => $mInfo->importers_email ?? '']))];
