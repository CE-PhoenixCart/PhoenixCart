    ...
    <div id="appPayPalToolbar">
      ...
      foreach ( $OSCOM_PayPal->getModules() as $m ) {
        if ( $OSCOM_PayPal->isInstalled($m) ) {
          echo $OSCOM_PayPal->drawButton($OSCOM_PayPal->getModuleInfo($m, 'short_title'), $GLOBALS['Admin']->link('paypal.php', ['action' => 'configure', 'module' => $m]), 'info', 'data-module="' . $m . '"') . "\n";
        }
      }
      echo $OSCOM_PayPal->drawButton($OSCOM_PayPal->getDef('section_general'), $GLOBALS['Admin']->link('paypal.php', ['action' => 'configure', 'module' => 'G']), 'info', 'data-module="G"');
      ?>
    </div>
  </div>
  <div class="col">
    ...
    <nav id="appPayPalToolbarMore" class="nav">
      ...
      foreach ( $OSCOM_PayPal->getModules() as $m ) {
        if ( !$OSCOM_PayPal->isInstalled($m) ) {
          echo '<a class="nav-link btn btn-sm btn-secondary mr-1" title="' . $OSCOM_PayPal->getModuleInfo($m, 'short_title') . '" href="' . $GLOBALS['Admin']->link('paypal.php', ['action' => 'configure', 'module' => $m]) . '">' . $m . '</a>';
        }
      }
      ?>
    </nav>
...
<p><?= $OSCOM_PayPal->drawButton($OSCOM_PayPal->getDef('button_install_title', array('title' => $OSCOM_PayPal->getModuleInfo($current_module, 'title'))), $GLOBALS['Admin']->link('paypal.php', ['action' => 'configure', 'subaction' => 'install', 'module' => $current_module]), 'success') ?></p>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a role="button" class="btn btn-danger" href="<?= $GLOBALS['Admin']->link('paypal.php', ['action' => 'configure', 'subaction' => 'uninstall', 'module' => $current_module]) ?>"><?= $OSCOM_PayPal->getDef('modal_uninstall_do_it') ?></a>
      </div>
    </div>
  </div>
</div>
