...
EOSQL
    ...
    $importer_inputs_string .= '<div class="input-group mb-1"><span class="input-group-text">' . $flag_image . '</span>' . new Input("importers_url[{$l['id']}]", ['id' => "mUrl-{$l['code']}", 'value' => $importer['importers_url']]) . '</div>';
    $importer_description_string .= '<div class="input-group mb-1"><span class="input-group-text">' . $flag_image->set('style', 'vertical-align: top;') . '</span>' . (new Textarea("importers_description[{$l['id']}]", ['cols' => '80', 'rows' => '10', 'id' => "mDescription-{$l['code']}"]))->set_text($importer['importers_description'] ?? '') . '</div>';
  }
